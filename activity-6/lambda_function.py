...
def create_file(
    filename, owner, shared_with=[], s3_key=None, size=0, last_modified=datetime.now()
):
    return {
        "filename": filename,
        "owner": owner,
    ...
    if command == "put":
        ...
        if username not in db["files"]:
            ...
            for file in db["files"]:
                if (
                    file["owner"] == username or username in file["shared_with"]
        ...
        if (file["owner"] == requester or requester in file["shared_with"]) and file[
            "filename"
        ] == target_file:
